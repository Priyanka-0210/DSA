//Merging two linkedlist using rearranging the pointer
#include<iostream>
using namespace std;
template<class T>
class Node{
...
void Chain<T>::merge(Node<T> *head1, Node<T> *head2){
        Node<T>* temp_head, temp_traverse;
        temp_head = nullptr;
        temp_traverse = nullptr;
        ...
        while(head1 != nullptr && head2 != nullptr){
            if(head1 -> data <= head2 -> data){
                temp_traverse -> next =  head1;
                temp_traverse = head1;
                head1 = head1 -> next;
            }
            else{
                temp_traverse -> next = head2;
                temp_traverse = head2;
                head2 = head2 -> next;
            }
            temp_traverse = nullptr;
